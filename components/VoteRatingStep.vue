  ...
  <div class="space-y-8">
    ...
    <div v-if="isLoading" class="space-y-5">
      ...
      >
        ...
        <div class="flex justify-center gap-3">
          <div v-for="j in 5" :key="j" class="w-10 h-10 bg-zinc-800 rounded-full"></div>
        </div>
      </div>
    </div>
    ...
    <div v-else class="space-y-5">
      <div
        ...
        :key="criterion.id"
        class="bg-zinc-900/50 backdrop-blur-sm rounded-2xl p-6 md:p-7 border-2 transition-all duration-300"
        :class="[
        ...
        <div class="space-y-4">
          ...
          <div class="flex items-center justify-center gap-2 md:gap-3">
            <button
              ...
              :class="[
                ratings[criterion.id] === star ? 'scale-110' : 'hover:scale-110',
              ]"
            ...
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                :fill="(hoverRating[criterion.id] || ratings[criterion.id]) >= star ? '#f6f2c0' : 'none'"
                :stroke="(hoverRating[criterion.id] || ratings[criterion.id]) >= star ? '#f6f2c0' : '#cb9b51'"
                stroke-width="2"
                class="w-10 h-10 md:w-12 md:h-12 transition-all duration-300"
                :class="[
                  (hoverRating[criterion.id] || ratings[criterion.id]) >= star
                    ? 'drop-shadow-[0_0_8px_rgba(246,242,192,0.5)]'
                    : 'group-hover:stroke-gold-400'
                ]"
              >
                <path
                  stroke-linecap="round"
          ...
          <div
            ...
            class="text-center py-3 rounded-xl bg-gradient-to-r from-gold-500/10 via-gold-500/20 to-gold-500/10 border border-gold-500/30 animate-in fade-in slide-in-from-bottom-2 duration-300"
          >
            <span class="text-sm text-gray-400">{{ $t('modal.yourRating') }} </span>
...
const { criteria: apiCriteria, fetchCriteria, isLoading, getCriterionLabel } = useCriteria()
...
const isAllRated = computed(() => {
  return displayCriteria.value.every((criterion) => ratings.value[criterion.id] > 0)
})
