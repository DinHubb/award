<template>
    ...
    <Dialog as="div" class="relative z-50" @close="handleClose">
      ...
      <div class="fixed inset-0 overflow-y-auto">
        ...
        >
          ...
          >
            <DialogPanel
              class="relative w-full h-full max-w-3xl overflow-y-auto bg-zinc-900 rounded-3xl border-2 border-zinc-800 shadow-2xl"
            >
              <!-- Close Button -->
              <button
                type="button"
                @click="handleClose"
                class="absolute max-sm:hidden top-2.5 right-2.5 z-10 p-2.5 rounded-xl text-gray-400 hover:text-white hover:bg-zinc-800/80 transition-all duration-300 hover:rotate-90 group"
                aria-label="Close"
              >
                <svg
                ...
                <div class="mb-14">
                  <div class="flex items-center justify-center gap-2 md:gap-3">
                    ...
                    <div class="flex flex-col items-center relative">
                      ...
                      <span
                        class="absolute -bottom-6 text-xs font-medium transition-colors duration-300"
                        :class="
                          currentStep === 1
                            ? 'text-gold-400'
                            : currentStep > 1
  ...
  <!-- OTP Alert -->
  <OtpAlert v-if="otpCode" :otp-code="otpCode" @close="otpCode = ''" />
</template>
...
const handleCodeSubmit = async (code: string) => {
  ...
  try {
    ...
    await confirmVote(phoneNumber.value, code);
